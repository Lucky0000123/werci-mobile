...
export default function NavMap(props: DispatchMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'route-cap', type: 'line', source: 'route', paint: { 'line-color': '#1D4ED8', 'line-width': 8, 'line-opacity': 0.95 }, layout: { 'line-cap': 'round', 'line-join': 'round' } })
      map.addLayer({ id: 'route-line', type: 'line', source: 'route', paint: { 'line-color': ['match', ['get', 'lane'], 'empty', '#39a0ff', 'loaded', '#ff5a3c', '#60A5FA'], 'line-width': 4.5 }, layout: { 'line-cap': 'round', 'line-join': 'round' } })
      setReady(true)
